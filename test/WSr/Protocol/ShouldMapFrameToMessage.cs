...
namespace WSr.Protocol.Tests
{
    ...
    public class ShouldMapFramesToMessages
    {
        ...
        private Dictionary<string, (Parse<FailedFrame, Frame> input, Message expected)> testcases
          = new Dictionary<string, (Parse<FailedFrame, Frame> input, Message expected)>()
          {
              ["BadFrame"] = (
                  input: Error(FailedFrame.ProtocolError("test")),
