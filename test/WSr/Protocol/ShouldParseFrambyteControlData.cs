...
namespace WSr.Protocol.Tests
{
    ...
    public class ShouldParseFramebyteControlData
    {
        ...
        private static Either<FrameByte> F(Guid id, bool fin, OpCode op, ulong f, byte b) => 
            new Either<FrameByte>(FrameByte.Init(H(id, fin, op)).With(@byte: b, followers: f));

        private static Either<FrameByte> F(Head h, ulong f, byte b) => 
            new Either<FrameByte>(FrameByte.Init(h).With(@byte: b, followers: f));  

        private (byte b, Either<FrameByte> f)[] FirstByteCases = new[]
        {
            ((byte)0x80, F(Id, true, OpCode.Continuation, 6, 0x80)),
            ((byte)0x81, F(Id, true, OpCode.Text, 6, 0x81)),
            ((byte)0x82, F(Id, true, OpCode.Binary, 6, 0x82)),
            ((byte)0x88, F(Id, true, OpCode.Close, 6, 0x88)),
            ((byte)0x89, F(Id, true, OpCode.Ping, 6, 0x89)),
            ((byte)0x8A, F(Id, true, OpCode.Pong, 6, 0x8A)),
            ((byte)0x00, F(Id, false, OpCode.Continuation, 6, 0x00)),
            ((byte)0x01, F(Id, false, OpCode.Text, 6, 0x01)),
            ((byte)0x02, F(Id, false, OpCode.Binary, 6, 0x02))
        };
        ...
        {
            ((byte)0x80, F(TextHead, 4, 0x80)),
            ((byte)0xFD, F(TextHead, 129, 0xFD)),
            ((byte)0xFE, F(TextHead, 6, 0xFE)),
            ((byte)0xFF, F(TextHead, 12, 0xFF))
        };
